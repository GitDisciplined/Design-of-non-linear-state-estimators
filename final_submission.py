...
action_vec=[10,20,30,40,100,200,300,800]
...
for episodes in range(1):
    ...
    #u=25
       ...
       # u=500
        ...
        if 50<=i<=65:
            ...
            D= np.random.normal(18,2)

        elif 250<=i<=265:

            D= np.random.normal(20,2)
    ...
    #tmax_ceil=state[1]/Ug_ceil
     ...
     #   tmax=temp
        ...
        EGP=max((1-call[9].item())*EGP0,0)
        Ug=call[1].item()/tmax
        G=max(call[2].item()/Vg,(35.69/Vg))
        ...
        #test=max(ukf.x[2]*7*np.exp(ukf.x[2]-tem[2]),0)+max(40*np.exp(-(ukf.x[9]-tem[9])),0).item()
        #print(test)
...
plt.plot(time,hns2,label='truth',color='red', linestyle='solid')
plt.plot(time,est1,label='ukf',color='green', linestyle='solid')
plt.legend()
...
plt.plot(time,hns3,label='truth',color='red', linestyle='solid')
plt.plot(time,est2,label='ukf',color='green', linestyle='solid')
plt.legend()
plt.title('Interstitial glucose concentration ')
plt.ylabel('Glucose concentration (mmol/L)')
plt.xlabel('time (minutes)')
plt.show()
...
plt.plot(time,ins,label='truth',color='red', linestyle='solid')
#plt.plot(time,est1,label='ukf',color='green', linestyle='solid')
plt.legend()
plt.title('Insulin infusion rate ')
plt.ylabel('Insulin rate(mU/minute)')
plt.xlabel('time (minutes)')
