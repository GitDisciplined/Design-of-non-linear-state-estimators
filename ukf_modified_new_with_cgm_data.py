...
def two_compartment(t,y):
    ...
    dQ1dt=-(k01+k21)*y[0]+k12*y[1]
    dQ2dt=k21*y[0]-(k12+k02)*y[1]
...
def next_state(state):
    t_span = (0, 1) 
    #noise1=np.random.multivariate_normal(np.array([0,0]),Q)
    #solution = solve_ivp(two_compartment, t_span, state+noise1)
    solution = solve_ivp(two_compartment, t_span, state)
...
def initialize():
    ...
    X0=np.array([149.02,165.7939])
    Q=np.diag([1,1])
    R=9
    P0=np.diag([400**2,400**2])
...
for itere in range(no_of_steps):
    ...
    X0=np.array([estimated_ns[0],ig[itere]])
    #X0=estimated_ns
...
plt.plot(time_steps,state_estimate1,label='est_bg')
plt.plot(time_steps,state_estimate2,label='est_ig')
plt.plot(time_steps,bg,label='cgm_bg')
plt.plot(time_steps,ig,label='cgm_ig')
plt.xlabel("time")
plt.ylabel("glucose concentration ")
plt.title("Estimation of glucose concentration")
plt.legend()
plt.show()
...
#plt.xlabel("time")
#plt.ylabel("glucose concentration on interstitial plasma")
#plt.title("Estimation of interstitial glucose")
#plt.legend()
#plt.show()
...
print('the mean sqared error for the estimation of bg is', np.mean(np.square(np.array(state_estimate1) - np.array(bg))))
