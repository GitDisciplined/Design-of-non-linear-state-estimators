...
def update(dim,actual_measure,pred_state,pred_covar,R):
   ...
   #cross_covariance
    ...
    for b2 in range(len(u_neg)):
        u_neg_cc.append(np.matmul(u_neg[b2]-pred_state,u_neg_tr[b2]-pred_measure)*com_weight_u)

    u_0_cc=  np.matmul(u_0_tr-pred_state,u_0_tr-pred_measure)* weight_cov_u
